...
!define PRODUCT_NAME "The Ur-Quan Masters High Definition"
...
; Uninstaller pages
!define MUI_UNCONFIRMPAGE_TEXT_TOP "This program will now uninstall The Ur-Quan Masters Project6014 entirely.  If you wish to preserve content or expansion packs, select Cancel now and back them up.  Otherwise, press Uninstall to continue."
...
OutFile "UQM-HD_v${PRODUCT_VERSION}_win_installer.exe"
...
Function .onInit
  ...
  StrCmp $0 "" NoAppData
  ExpandEnvStrings $UQMUSERDATA "%APPDATA%\uqmmod"
  Goto GotAppData
NoAppData:
  ReadEnvStr $0 USERPROFILE
  StrCmp $0 "" NoProfile
  ExpandEnvStrings $UQMUSERDATA "%USERPROFILE%\Application Data\uqmmod"
  Goto GotAppData
NoProfile:
  StrCpy $UQMUSERDATA "$INSTDIR\userdata\uqmmod"
GotAppData:
FunctionEnd
...
;      CopyFiles "$INSTDIR\content\packages\p6014-0.2.0-3domusic.uqm" "$INSTDIR\content\packages\p6014-0.6.0-3domusic.uqm"
;DelOldContent:
...
;    Delete "$INSTDIR\content\packages\p6014-0.1.1-content.uqm"
  ...
  Section "Core Data" SEC02
    SectionIn 1 2 3 4 6
    CreateDirectory "$INSTDIR\content\packages"
...
Section -ShortcutsAndIcons
  ...
  !insertmacro MUI_STARTMENU_WRITE_BEGIN Application
    ...
    CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\The Ur-Quan Masters p6014.lnk" "$INSTDIR\p6014.exe" $UQMARGS
    CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\p6014 (Safe Mode).lnk" "$INSTDIR\p6014.exe" "-x --safe"
    CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\p6014 (Safe OpenGL).lnk" "$INSTDIR\p6014.exe" "-o --safe"
...
;    CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\Keyboard Test.lnk" "$INSTDIR\keyjam.exe"
    ...
    CreateShortCut "$DESKTOP\The Ur-Quan Masters p6014.lnk" "$INSTDIR\p6014.exe" $UQMARGS
NoIcon:
...
Section -Post
  ...
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\p6014.exe"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
  !insertmacro MUI_DESCRIPTION_TEXT ${SECGRP01} "The core executables and content libraries for The Ur-Quan Masters Project6014.  All elements in this section must be installed for the game to be playable."
  !insertmacro MUI_DESCRIPTION_TEXT ${SEC01} "Includes the main program, all subsidiary libraries, and basic documentation for The Ur-Quan Masters Project6014. Required for play."
  ...
  !insertmacro MUI_DESCRIPTION_TEXT ${SEC06} "Optional package containing new voiceovers for The Ur-Quan Masters Project6014.  If this package is selected and not present in the packages directory, the installer will attempt to download it."
...
;  Delete "$INSTDIR\content\packages\addons\remix\p6014-remix-pack4.zip"
...
;  Delete "$SMPROGRAMS\$ICONS_GROUP\Keyboard Test.lnk"
  ...
  Delete "$DESKTOP\The Ur-Quan Masters HD: Alpha 1.lnk"
  Delete "$SMPROGRAMS\$ICONS_GROUP\The Ur-Quan Masters HD.lnk"
  Delete "$SMPROGRAMS\$ICONS_GROUP\uqmhd (Safe Mode).lnk"
  Delete "$SMPROGRAMS\$ICONS_GROUP\UQM-HD (Safe OpenGL).lnk"
...
;  RMDir "$INSTDIR\content\addons\remix"
